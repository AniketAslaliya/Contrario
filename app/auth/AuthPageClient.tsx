...
export function AuthPageClient({
  ...
  const [error, setError] = useState(
    callbackError === "callback"
      ? "Email link expired or failed. Request a new link below."
      : ""
  );
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim() || !magicLinkEnabled) return;
  ...
  return (
    <main className="relative min-h-screen flex items-center justify-center px-6">
      <div className="w-full max-w-md">
        <Link
          href="/"
          className="inline-flex items-center gap-2 text-sm text-ink-600 hover:text-ink transition-colors duration-300 mb-10"
        >
          <svg
            className="w-4 h-4"
            fill="none"
        ...
        <div className="card !rounded-3xl !p-10">
          ...
          {!magicLinkEnabled ? (
            <div className="p-4 rounded-2xl bg-cream-100 border border-cream-400 text-sm text-ink-700 leading-relaxed">
              Email sign-in needs{" "}
              <span className="font-medium text-ink">NEXT_PUBLIC_SUPABASE_URL</span>{" "}
              and{" "}
              <span className="font-medium text-ink">
                NEXT_PUBLIC_SUPABASE_ANON_KEY
              </span>{" "}
              in your environment. In Supabase → Authentication → URL Configuration, add redirect:{" "}
            ...
            <div className="text-center py-2">
              <p className="font-serif text-2xl text-ink mb-3">Check your email</p>
              ...
              <Link
                ...
                className="text-sm font-medium text-ink hover:text-persona-scale transition-colors"
              >
            ...
            <>
              {googleEnabled ? (
                <>
                  ...
                  <div className="flex items-center gap-4 my-6">
                    ...
                    <span className="text-xs text-ink-600 uppercase tracking-[0.15em]">
                      or
                    </span>
                    <div className="h-[1px] flex-1 bg-cream-400" />
                  </div>
              ...
              <form onSubmit={handleMagicLink}>
                ...
                <input
                  ...
                  required
                  autoComplete="email"
                  className="w-full px-4 py-3 rounded-2xl bg-cream-100 border border-cream-400 text-ink placeholder:text-ink-500 focus:outline-none focus:border-persona-scale focus:ring-1 focus:ring-persona-scale/25 transition-all"
                />
                <button
                  type="submit"
          ...
          <div className="mt-8 pt-6 border-t border-cream-400 text-center">
            <p className="text-sm text-ink-600 mb-2">Just want to try it?</p>
            <Link
              href="/analyze"
